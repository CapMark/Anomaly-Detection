...
class TorchModel(nn.Module):
    ...
    def evaluate(self, criterion):
        features_path = "features\c3d" #sostituire con path per c3d features
        data_loader = DataloaderVal(features_path, annotation_path="Test_Annotation.txt")
        data_iter = torch.utils.data.DataLoader(data_loader, batch_size=1, shuffle=False, num_workers=0,
                                                pin_memory=True)
        batches = torch.FloatTensor(280, 32, 4096)
        targets = torch.FloatTensor(280)
        i=0
        self.eval()
        with torch.no_grad():
